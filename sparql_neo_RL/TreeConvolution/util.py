...
def fix_flatten_trees(flat_trees):
    max_sh = 0
    for tree in flat_trees:
        for subtree in tree:
            #print(subtree.shape[0])
            if len(subtree.shape) != 0:
                if subtree.shape[0] > max_sh:
                    max_sh = subtree.shape[0]                
    for tree in range(len(flat_trees)):
        for subtree in range(len(flat_trees[tree])):
            if len(flat_trees[tree][subtree].shape) == 0:
                flat_trees[tree][subtree] = np.zeros((max_sh,), dtype=np.float64)
    
    for tree in range(len(flat_trees)):
        aux = []
        if flat_trees[tree].dtype == np.object:
            for subtree in range(len(flat_trees[tree])):
                aux.append(flat_trees[tree][subtree])
            
            #flat_trees[tree] = flat_trees[tree].astype(np.float64)
            #print('flat_trees[tree]',flat_trees[tree])
            #print('flat_trees[tree][0]',flat_trees[tree][0])
            #print('type(flat_trees[tree]),flat_trees[tree].dtype',type(flat_trees[tree]),flat_trees[tree].dtype)
            #print('type(flat_trees[tree][0]),flat_trees[tree][0].dtype',type(flat_trees[tree][0]),flat_trees[tree][0].dtype)
            #print('len(flat_trees[tree])',len(flat_trees[tree]))
            #print('len(flat_trees[tree][0])',len(flat_trees[tree][0]))
            
            #print("ñññññññññññññññññññ CON EL aux ñññññññññññññññññññññññññññ")
            aux = np.asarray(aux, dtype=np.float64)
            #print((flat_trees[tree] == aux).all())
            flat_trees[tree] = aux
            #print('flat_trees[tree]',flat_trees[tree])
            #print('flat_trees[tree][0]',flat_trees[tree][0])
            #print('type(flat_trees[tree]),flat_trees[tree].dtype',type(flat_trees[tree]),flat_trees[tree].dtype)
...
def prepare_trees(trees, transformer, left_child, right_child, cuda=False):
    ...
    for i in trees:
        print("||||||||||||||||||||")
        print(t,type(i),len(i))
        print("||||||||||||||||||||")
        for ii in i:
            if isinstance(ii, tuple):
                print(t,type(ii),len(ii))
            else:
                print(t,type(ii),len(ii),ii.dtype,ii.shape,len(ii.shape))
        t +=1
    print("------------------FLAT TREES--------------------")
    print("------------------FLAT TREES--------------------")
    print("------------------FLAT TREES--------------------")
    ...
    flat_trees = fix_flatten_trees(flat_trees)
